...
每個產物的觸發條件對應**痛點第一次出現**的時機:

- **起手三檔**(`CLAUDE.md` + `init.sh` + `feature_list.json`)解決「範圍失控」與「假完成」—— 專案一開始就會痛,所以動工日必建。
- **交接檔**解決「跨 session 失憶」—— 第一次沒做完就收工才會痛,所以那時才建。
- **回饋與驗證產物**解決「難查的 bug」與「同型錯誤重複發生」—— 事故發生了才痛,不是功能一多就痛。

早期版本把這些排成 L1/L2/L3 三級,配一套升降級協定。數週實際使用下來,分級儀式本身沒有改變任何行為——真正在做事的是每個產物自己的觸發條件——所以層級拿掉了,觸發條件留下。

規則:**痛點出現前,不要預建對應的組件。** 小工具一輩子只靠起手三檔是正確的,不是偷懶。
...
所以驗收固定交給**另一個 fresh-context 的 agent**,而且只餵兩樣東西:**PRD + 成品**,不給開發過程。驗收者不知道實作者「為什麼這樣寫」,只能對照 acceptance 逐條檢查「做到了沒、證據在哪」。
...
沒發揮作用的組件,下個專案不要照抄。這一步防止 harness 本身變成 cargo-cult —— 框架也要接受「被自己的標準檢驗」。若有啟用結構化 trace,這個判斷可以用實際數據(攔了幾次越權、驗收 fail 了哪些)佐證,而不是靠印象。
