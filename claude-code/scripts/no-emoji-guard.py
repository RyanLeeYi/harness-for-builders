...
# Windows 主控台／pipe 預設 cp950：不釘 UTF-8 的話，stdin 讀到中文或 emoji 就 UnicodeDecodeError，
# 被 main() 的 except 吞掉變成永遠放行——一道無聲倒塌的牆比沒有牆更危險。stderr 同理，
# 擋下來要印的中文訊息會直接 UnicodeEncodeError。
...
# Obsidian vault 整棵放行：實測 157 個 .md 含 emoji，其中 knowledge/、career/、成就故事/
# 都是既有寫法，agent-persona.md 的 Cub 熊頭是人格識別，.roam-sync/ 是外部原文 staging。
# 不放行的話每次寫 vault 都會被擋。emoji 禁令的目標是 code repo 與正式文件，不是筆記庫。
# Windows payload 的 file_path 用反斜線，但 Git Bash 情境可能給正斜線，兩種都列。
# 由環境變數 NO_EMOJI_EXEMPT 設定（分號分隔的路徑子字串），沒設就不放行任何路徑。
# 例：NO_EMOJI_EXEMPT="/my-notes/;\my-notes\" 放行你的筆記庫
import os
EXEMPT_PATH_SUBSTRINGS = [s for s in (os.environ.get("NO_EMOJI_EXEMPT") or "").split(";") if s]
# ───────────────────────────────────────────────────────────────────────
